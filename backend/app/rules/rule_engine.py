...
class RuleEngine:
    ...
    def evaluate(
        ...
        if advance_percent > settings.max_advance_payment_percent:
            risks.append(
                RiskItem(
                    ...
                    message=f"预付款比例 {advance_percent}% 超过红线 {settings.max_advance_payment_percent}%",
                    severity=RiskSeverity.high,
                    location="合同专用条款 > 付款条款",
                )
            )
        ...
        if delivery_days > 0 and "一次性验收" in acceptance_text and int(structured_data.get("delivery_batches") or 1) > 1:
            risks.append(
                RiskItem(
        ...
        day_values = {int(value) for value in re.findall(r"(\d+)\s*天", rendered_content or "")}
        if len(day_values) > 1:
            risks.append(
                RiskItem(
